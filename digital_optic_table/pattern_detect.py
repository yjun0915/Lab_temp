...
lines = cv2.HoughLinesP(image=canny_image, rho=10, theta=np.pi/3600, threshold=160, minLineLength=0, maxLineGap=0)
'''
...
max_radius = 30
score = np.zeros(shape=(max_radius, 2))
cv2.namedWindow(winname="convoluting")
for radi in range(1, max_radius):
    circle = np.zeros(shape=(radi*2, radi*2))
    cv2.circle(img=circle, center=(radi-1, radi-1), radius=radi, color=[1], thickness=1)

    convoluted_map = cv2.filter2D(src=canny_image/255, ddepth=-1, kernel=circle, delta = -10)/280
    gaussian_map = cv2.GaussianBlur(src=convoluted_map, ksize=(5, 5), sigmaX=0.1)*10
    gaussian_map = gaussian_map.astype(np.int64)*1.0

    score[radi] = [radi, np.sum(gaussian_map)]

    print("[%3d]"%radi, "Sum of convoluted map :", score[radi][1])
    cv2.imshow(winname="convoluting", mat=gaussian_map)
    cv2.waitKey(10)
ax1 = fig1.add_subplot(111)
ax1.plot(score)

plt.waitforbuttonpress()
radius = int(score[score.argmax(axis=0)[1]][0])
print("Estimated radius of holes :", radius)
...
for ix in range(int((np.shape(magnitude_spectrum)[0] - 2*radius)/2),
                int((np.shape(magnitude_spectrum)[0] + 2*radius)/2)):
    for iy in range(int((np.shape(magnitude_spectrum)[1] - 2*radius) / 2),
                    int((np.shape(magnitude_spectrum)[1] + 2*radius) / 2)):
        magnitude_spectrum[ix][iy] = 0

fft_plot = smooth(magnitude_spectrum[int(np.shape(magnitude_spectrum)[0]/2), :], 12)
