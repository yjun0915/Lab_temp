...
for m in range(int(m_max / 2)):
    for n in range(n_max):
        ...
        if LG:
            ...
            s = f'$HG_{n}$' + f'$_{m}$'
        axs[m + i][n].imshow(I, cmap='jet');
        axs[m + i][n].axis('off');
        axs[m + i][n].set_title(s)
        axs[m + 1 + i][n].imshow(Phi, cmap='rainbow');
        axs[m + 1 + i][n].axis('off');
    i += 1
